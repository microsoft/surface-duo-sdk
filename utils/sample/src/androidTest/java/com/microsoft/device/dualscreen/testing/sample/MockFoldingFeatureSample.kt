...
import com.microsoft.device.dualscreen.testing.filters.MockFoldingFeature.FoldingFeatureOrientation.HORIZONTAL
import com.microsoft.device.dualscreen.testing.filters.MockFoldingFeature.FoldingFeatureState.FLAT
...
class MockFoldingFeatureSample {
    ...
    @Test
    @MockFoldingFeature(
        windowBounds = [0, 0, 1000, 1000],
        size = 10,
        state = FLAT,
        orientation = HORIZONTAL
    )
    @TargetDevices(ignoreDevices = [SurfaceDuo, SurfaceDuo2])
    fun testOn10PxFoldingFeature() {
