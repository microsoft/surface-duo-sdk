...
class DialogFragmentNavigatorDestinationBuilderTest {
    ...
    @UiThreadTest
    @Test fun fragment() {
        val navHostFragment = FoldableNavHostFragment()
        ...
        assertWithMessage("Destination should be added to the graph")
            .that(DESTINATION_ID in graph.map { it.id })
            .isTrue()
        assertWithMessage("DialogFragment class should be set to BuilderTestDialogFragment")
            .that((graph[DESTINATION_ID] as FoldableDialogFragmentNavigator.Destination).className)
