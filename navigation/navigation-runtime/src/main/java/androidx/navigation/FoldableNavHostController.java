...
/**
 * Subclass of {@link FoldableNavController} that offers additional APIs for use by a
 * {@link FoldableNavHost} to connect the NavController to external dependencies.
 *
 * <p>Apps should generally not construct controllers, instead obtain a relevant controller
 * directly from a navigation host via {@link FoldableNavHost#getNavController()} or by using one of
 * the utility methods on the {@link FoldableNavigation} class.</p>
 */
public class FoldableNavHostController extends FoldableNavController {
    /**
     * Construct a new controller for a given {@link Context} suitable for use in a
     * {@link FoldableNavHost}. Controllers should not be used outside of their context and retain a
     ...
     */
    public FoldableNavHostController(@NonNull Context context) {
        super(context);
    }
    /**
     * Sets the host's {@link LifecycleOwner}.
     *
     * @param owner The {@link LifecycleOwner} associated with the containing {@link FoldableNavHost}.
     ...
     * should generally be called for you by your {@link FoldableNavHost}.
     * <p>
     * You must call this method before {@link #setGraph(int)} or similar methods, because the
     * {@link ViewModelStore} set here will be used by the created {@link FoldableNavBackStackEntry} items.
     *
