...
class DuoTabLayout : TabLayout {
    ...
    private fun initialize(attrs: AttributeSet) {
        val styledAttributes =
            context.theme.obtainStyledAttributes(attrs, R.styleable.DuoTabLayout, 0, 0)
        try {
            displayPosition = DisplayPosition.fromId(
                styledAttributes.getInt(
                    R.styleable.DuoTabLayout_display_position,
                    DisplayPosition.DUAL.ordinal
                )
            )
            screenMode = ScreenMode.fromId(
                styledAttributes.getResourceId(
                    R.styleable.DuoTabLayout_tools_application_mode,
