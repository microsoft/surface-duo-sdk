...
package com.microsoft.device.dualscreen.bottmnavigation
...
import com.microsoft.device.dualscreen.common.isPortrait
import com.microsoft.device.dualscreen.common.isSpannedInDualScreen
import com.microsoft.device.dualscreen.layout.R
 ...
 */
open class DuoBottomNavigationView @JvmOverloads constructor(
    context: Context,
    ...
    var duoDisplayPosition: DisplayPosition
        get() {
    ...
    private fun extractAttributes(context: Context, attrs: AttributeSet?) {
        val styledAttributes =
            context.theme.obtainStyledAttributes(attrs, R.styleable.DuoBottomNavigationView, 0, 0)
        try {
            displayPosition = DisplayPosition.fromId(
                styledAttributes.getInt(
                    R.styleable.DuoBottomNavigationView_display_position,
                    DisplayPosition.DUAL.ordinal
                )
            )
            screenMode = ScreenMode.fromId(
                styledAttributes.getResourceId(
                    R.styleable.DuoBottomNavigationView_tools_application_mode,
